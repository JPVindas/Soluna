...
export function CatalogFilters({filters,onChange,items=products,activeCount,onClear}:{filters:Filters;onChange:(f:Filters)=>void;items?:Product[];activeCount?:number;onClear?:()=>void}){
 ...
 function clear(){if(onClear)onClear();else onChange(emptyFilters());setBrandQuery('')}
 return <div className="catalog-filters"><div className="filters-heading"><h2><SlidersHorizontal size={17}/>{t('Filtros','Filters')} <span>({activeCount??filterCount(filters)})</span></h2></div><p className="filter-catalog-label">{t('Encuentra tu esencia','Find your essence')}</p><Accordion defaultValue={['price','brands','sizes','gender']} multiple>
